...
class BadRequestException extends IHttpException {
  const BadRequestException({HttpStatus? status, super.data, super.message = ''}) : super(status ?? HttpStatus.badRequest);
}

class ServerErrorException extends IHttpException {
  const ServerErrorException({super.message = ''}) : super(HttpStatus.serverError);
}

class UnprocessableEntityException extends IHttpException {
  const UnprocessableEntityException({super.data, super.message = ''}) : super(HttpStatus.unprocessableEntity);
}

class UnauthorizedException extends IHttpException {
  const UnauthorizedException({super.data, super.message = ''}) : super(HttpStatus.unauthorized);
}

class TimeoutException extends IHttpException {
  const TimeoutException({super.message = ''}) : super(HttpStatus.requestTimeout);
}
